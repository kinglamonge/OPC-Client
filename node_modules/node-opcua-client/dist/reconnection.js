    ...
    async.series([
        ...
        function activate_new_session(innerCallback) {
            ...
            client._activateSession(newSession, newSession.userIdentityInfo, (err, session1) => {
                ...
                if (err) {
                    doDebug && debugLog(chalk.bgWhite.cyan("reactivation of the new session has failed: let be smart and close it before failing this repair attempt"));
        ...
        function attempt_subscription_transfer(innerCallback) {
            ...
            newSession.transferSubscriptions(subscriptionsToTransfer, (err, transferSubscriptionsResponse) => {
                ...
                // istanbul ignore next
                if (doDebug) {
                    doDebug && debugLog(chalk.cyan("    =>  transfer subscriptions  done"), results.map((x) => x.statusCode.toString()).join(" "));
                }
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (statusCode.equals(node_opcua_status_code_1.StatusCodes.BadSubscriptionIdInvalid)) {
                        // repair subscription
                        doDebug && debugLog(chalk.red("         WARNING SUBSCRIPTION  "), subscriptionsIds[i], chalk.red(" SHOULD BE RECREATED"));
                        subscriptionsToRecreate.push(subscriptionsIds[i]);
                    }
                    else {
                        const availableSequenceNumbers = results[i].availableSequenceNumbers;
                        doDebug && debugLog(chalk.green("         SUBSCRIPTION "), subscriptionsIds[i], chalk.green(" CAN BE REPAIRED AND AVAILABLE "), availableSequenceNumbers);
                        // should be Good.
                    }
                }
                ...
                async.forEach(subscriptionsToRecreate, (subscriptionId, next) => {
                    ...
                    subscription.recreateSubscriptionAndMonitoredItem((err1) => {
                        ...
                        doDebug && debugLog(chalk.cyan("          => RECREATING SUBSCRIPTION  AND MONITORED ITEM DONE "), subscriptionId);
                        next();
                    });
                }, (err1) => {
                    innerCallback(err1);
